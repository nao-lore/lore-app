#!/usr/bin/env bash
# check-freeze.sh — PreToolUse hook for freeze boundary enforcement (Lore)
# Based on gstack /freeze. Reads JSON from stdin, checks if file_path is within the freeze boundary.
...
FREEZE_FILE="$HOME/.lore/freeze-dir.txt"
